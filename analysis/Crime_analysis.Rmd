...
head(NA_table, 5)
>>>>>>> 3b4276ccf3cc3194afbea85582c619387c6d5579
```
...
sort(table(data_reduction$OFNS_DESC_NEW), decreasing = TRUE)
data_mosaic <- subset(data_reduction, OFNS_DESC_NEW == "ROBBERY/BURGLARY/LARCENY" | OFNS_DESC_NEW == "OFFENSES AGAINST THE PERSON"| OFNS_DESC_NEW == "UNCLASSIFIED STATE LAWS VIOLATION"| OFNS_DESC_NEW == "OFFENSES AGAINST PUBLIC ADMINI"| OFNS_DESC_NEW == "CRIME RELATED TO HEALTH")
head(data_mosaic)

#data_mosaic <- data_mosaic[-which(data_reduction$SUSP_SEX == "NA"|data_reduction$SUSP_SEX == ""), ]
#data_mosaic <- data_mosaic[-which(data_reduction$SUSP_AGE_GROUP == "UNKNOWN"|data_reduction$SUSP_AGE_GROUP == ""), ]

data_mosaic$SUSP_SEX <- as.factor(data_mosaic$SUSP_SEX)
data_mosaic$SUSP_AGE_GROUP <- as.factor(data_mosaic$SUSP_AGE_GROUP)
data_mosaic$OFNS_DESC_NEW <- as.factor(data_mosaic$OFNS_DESC_NEW)
...
xmarks <- c("", "", "")
mosaic(SUSP_SEX~ SUSP_AGE_GROUP + OFNS_DESC_NEW,data = data_mosaic,
            direction = c("v", "v", "h"), 
            gp = gpar(fill = icecreamcolors))
ggplot(data = data_mosaic) + 
  geom_mosaic(aes(x = product(OFNS_DESC_NEW, SUSP_AGE_GROUP, SUSP_SEX), fill=OFNS_DESC_NEW, conds=product(BORO_NM)), na.rm=TRUE, divider=mosaic("v"))+facet_grid(SUSP_SEX~.) +
